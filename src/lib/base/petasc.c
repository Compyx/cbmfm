 ...
 * cbmfm_pet_to_asc() on a value returned by cbmfm_asc_to_pet() may not return the
 * original value passed to cbmfm_asc_to_pet(). For example, ASCII has CR, LF, FF
 * while PETSCII has just CR, so cbmfm_pet_to_asc(cbmfm_asc_to_pet(0x0a)) will return
 * 0x0d, not 0x0a.
 */
 ...
 *
 * In UNIX(-like) systems, just about evething is allowed with escaping, except
 * the forward slash, in Windows systems there's quite a few disallowed chars.
 */
static const char *cbmfm_host_illegal_chars =
...
{
    const char *t = cbmfm_host_illegal_chars;
    while (*t != '\0' && ch != *t) {
        t++;
    }
...
void cbmfm_pet_filename_to_host(char *asc, const uint8_t *pet, const char *ext)
{
    ...
    if (lead < trail) {
        ...
        for (i = 0; i < trail - lead; i++) {
            int b = cbmfm_pet_to_asc(pet[lead + i]);
            asc[i] = isprint(b) && cbmfm_is_host_allowed_char(b)
                ? (char)b : '_';
        }
        /* terminate string */
        asc[trail - lead] = '\0';
